...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        $schedule->call(function () {
            ...
            // By genre bars
            $bars_by_genre = "https://quickchart.io/chart?w=500&h=300&c={type:%27bar%27,data:{labels:['Mujeres','Hombres'],datasets:[{labels:%27Genero%27,data:[" . $mujeres . "," . $hombres . "]}]}}";
            ...
            ////By Company type percent
            $chart_per_certificate_state = "https://quickchart.io/chart?c={type:'doughnut',data:{labels:['Ingresado','Solicitado','Aprobado','No Aprobado','Certificado','Documentado','Historico','Completo','En Proceso','No Conforme','Inactivo'],datasets:[{data:[";
                foreach ($percent_company_per_certificate_state as $key => $value) {
                    if ($key > 1) {
                        $chart_per_certificate_state.= ',';
                    }
                    $chart_per_certificate_state.= "'" . round($value, 2) . "'";
                }
            $chart_per_certificate_state.= "]}]},options:{plugins:{doughnutlabel:{labels:[{text:'" . $total_companies . "',font:{size:20}},{text:'total'}]}}}}";
            ///By company type bars total count
            ...
            /// cantidad de observaciones por contratista ///////////
        ...
        })->everyFiveMinutes(); ///Cambiar para prod
    }
